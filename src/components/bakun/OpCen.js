      ...
      >
        ...
        >
          ...
          <Grid item xs={7}>
            ...
            {!isRoutine && (
              <Typography variant="h6" style={{ textAlign: "center" }}>
                Validity: {moment(alert_validity).format("LL, h:mm A")}
              </Typography>
            )}
          </Grid>
          <Grid item xs={3}>
...
const OpCen = () => {
  const [latestCandidatesAndAlerts, setLatestCandidatesAndAlerts] =
    useState(null);
      ...
      if (element.hasOwnProperty("trigger_list_arr")) {
        element["trigger_list_arr"].forEach((el) => {
          temp.push({
            ...
            waray_tech_info: "",
            source:
              element.site_code === "mar" ? "Leon, MDRRMO" : element.site_code,
          });
        });
      }
    });
  ...
  useEffect(() => {
    if (latestCandidatesAndAlerts != null) {
      setCandidateList(
        JSON.parse(latestCandidatesAndAlerts["candidate_alerts"])
      );
  ...
  useEffect(() => {
    if (monitoring_releases) {
      if (
        ...
        monitoring_releases.overdue.length != 0
      ) {
  ...
  const getCurrentAlert = () => {
    if (monitoring_releases.latest.length != 0) {
      ...
      setValidityLabel(
        moment(monitoring_releases.overdue[0].event.validity).format("LLL")
      );

      let template = ewiTemplate.find(
        (x) =>
          x.alert_level ==
          monitoring_releases.overdue[0].highest_event_alert_level
      );
      if (template != -1) {
  ...
  return (
    <Fragment>
      ...
      {monitoring_releases != undefined &&
      monitoring_releases.hasOwnProperty("latest") ? (
        monitoring_releases.latest.length != 0 ? (
